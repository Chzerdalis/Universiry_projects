...
extern "C" void apatb_compute_matrices_hw(volatile void * __xlx_apatb_param_query, volatile void * __xlx_apatb_param_database, volatile void * __xlx_apatb_param_max_index, volatile void * __xlx_apatb_param_direction_matrix) {
    ...
    } // end post check logic bolck
  {
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<16> > direction_matrix_pc_buffer(2099136);
          int i = 0;
          ...
          if (i > 0) {{
            int i = 0;
            for (int j = 0, e = 2099136; j < e; j += 1, ++i) {
            ((short*)__xlx_apatb_param_direction_matrix)[j] = direction_matrix_pc_buffer[i].to_int64();
          }}}
        } // end transaction
      } // end file is good
    } // end post check logic bolck
...
// print query Transactions
{
  ...
  {  __xlx_offset_byte_param_query = 0*1;
  if (__xlx_apatb_param_query) {
    for (int j = 0  - 0, e = 32 - 0; j != e; ++j) {
sc_bv<8> __xlx_tmp_lv = ((char*)__xlx_apatb_param_query)[j];
  ...
  tcl_file.set_num(32, &tcl_file.query_depth);
...
// print database Transactions
{
  ...
  {  __xlx_offset_byte_param_database = 0*1;
  if (__xlx_apatb_param_database) {
    for (int j = 0  - 0, e = 65598 - 0; j != e; ++j) {
sc_bv<8> __xlx_tmp_lv = ((char*)__xlx_apatb_param_database)[j];
  ...
  tcl_file.set_num(65598, &tcl_file.database_depth);
  sprintf(__xlx_sprintf_buffer.data(), "[[/transaction]] \n");
  aesl_fh.write(AUTOTB_TVIN_database, __xlx_sprintf_buffer.data());
}
...
// print direction_matrix Transactions
{
  ...
  {  __xlx_offset_byte_param_direction_matrix = 0*2;
  if (__xlx_apatb_param_direction_matrix) {
    for (int j = 0  - 0, e = 2099136 - 0; j != e; ++j) {
sc_bv<16> __xlx_tmp_lv = ((short*)__xlx_apatb_param_direction_matrix)[j];
  ...
  tcl_file.set_num(2099136, &tcl_file.direction_matrix_depth);
  sprintf(__xlx_sprintf_buffer.data(), "[[/transaction]] \n");
  aesl_fh.write(AUTOTB_TVIN_direction_matrix, __xlx_sprintf_buffer.data());
}
