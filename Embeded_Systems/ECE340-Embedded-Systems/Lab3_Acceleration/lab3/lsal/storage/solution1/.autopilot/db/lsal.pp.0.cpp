...
# 3 "storage/lsal.cpp" 2
# 1 "storage/lsal.hpp" 1
# 19 "storage/lsal.hpp"
...
#pragma HLS BIND_STORAGE variable=direction_buff type=ram_t2p impl=bram
 ...
 MEM_1: memset(diag_array_1, 0, sizeof(char) * ((32)+1));
  ...
  MEM_5: memcpy(database_buff + 1, database, sizeof(char) * (32));
# 59 "storage/lsal.cpp"
...
#pragma HLS PIPELINE
 ...
 VITIS_LOOP_65_1: for(i = 1; i < (32); i++) {
       database_buff[i - 1] = database_buff[i];
      }
      database_buff[(32)-1] = database[k + (32) - 1];

         INNER_LOOP: for (i = 0; i < (32); i++) {


             j = (32) - 1 - i;
             signed char match = (querry_buff[j] == database_buff[i]) ? 2 : -1;
