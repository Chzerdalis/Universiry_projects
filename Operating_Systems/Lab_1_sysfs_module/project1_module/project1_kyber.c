 ...
 */
 ...
 static int team351635313269_kyber_domain_wake(wait_queue_entry_t *wait, unsigned mode, int flags,
				  void *key);
 ...
 static void team351635313269_kyber_resize_domain(struct kyber_queue_data *kqd,
 ...
 static void team351635313269_kyber_timer_fn(struct timer_list *t)
 {
	 ...
	 for (sched_domain = 0; sched_domain < KYBER_OTHER; sched_domain++) {
		 ...
		 if (bad || p99 >= KYBER_GOOD_BUCKETS) {
			 ...
			 team351635313269_kyber_resize_domain(kqd, sched_domain, depth);
		 }
	 }
 }
 
 static struct kyber_queue_data *team351635313269_kyber_queue_data_alloc(struct request_queue *q)
 {
	 ...
	 timer_setup(&kqd->timer, team351635313269_kyber_timer_fn, 0);
 ...
 static int team351635313269_kyber_init_sched(struct request_queue *q, struct elevator_type *e)
 {
	 struct kyber_queue_data *kqd;
	 ...
	 kqd = team351635313269_kyber_queue_data_alloc(q);
	 if (IS_ERR(kqd)) {
		 kobject_put(&eq->kobj);
		 return PTR_ERR(kqd);
	 }
 ...
 static void team351635313269_kyber_exit_sched(struct elevator_queue *e)
 {
 ...
 static void team351635313269_kyber_ctx_queue_init(struct kyber_ctx_queue *kcq)
 {
	 unsigned int i;
 ...
 static void team351635313269_kyber_depth_updated(struct blk_mq_hw_ctx *hctx)
 {
 ...
 static int team351635313269_kyber_init_hctx(struct blk_mq_hw_ctx *hctx, unsigned int hctx_idx)
 {
	 ...
	 for (i = 0; i < hctx->nr_ctx; i++)
		 team351635313269_kyber_ctx_queue_init(&khd->kcqs[i]);
	 ...
	 for (i = 0; i < KYBER_NUM_DOMAINS; i++) {
		 ...
		 init_waitqueue_func_entry(&khd->domain_wait[i].wait,
					   team351635313269_kyber_domain_wake);
	 ...
	 hctx->sched_data = khd;
	 team351635313269_kyber_depth_updated(hctx);
 ...
 static void team351635313269_kyber_exit_hctx(struct blk_mq_hw_ctx *hctx, unsigned int hctx_idx)
 {
 ...
 static void team351635313269_kyber_limit_depth(blk_opf_t opf, struct blk_mq_alloc_data *data)
 {
	 /*
 ...
 static bool team351635313269_kyber_bio_merge(struct request_queue *q, struct bio *bio,
 ...
 static void team351635313269_kyber_prepare_request(struct request *rq)
 {
	 rq_set_domain_token(rq, -1);
 }
 
 static void team351635313269_kyber_insert_requests(struct blk_mq_hw_ctx *hctx,
 ...
 static void team351635313269_kyber_finish_request(struct request *rq)
 {
 ...
 static void team351635313269_kyber_completed_request(struct request *rq, u64 now)
 {
	 struct kyber_queue_data *kqd = rq->q->elevator->elevator_data;
	 struct kyber_cpu_latency *cpu_latency;
	 unsigned int sched_domain;
 ...
 static void team351635313269_kyber_flush_busy_kcqs(struct kyber_hctx_data *khd,
				   unsigned int sched_domain,
				   struct list_head *list)
 {
 ...
 static int team351635313269_kyber_domain_wake(wait_queue_entry_t *wqe, unsigned mode, int flags,
				  void *key)
 {
 ...
 static int team351635313269_kyber_get_domain_token(struct kyber_queue_data *kqd,
				   struct kyber_hctx_data *khd,
 ...
 team351635313269_kyber_dispatch_cur_domain(struct kyber_queue_data *kqd,
			   struct kyber_hctx_data *khd,
			   struct blk_mq_hw_ctx *hctx)
 {
	 ...
	 if (rq) {
		 nr = team351635313269_kyber_get_domain_token(kqd, khd, hctx);
		 if (nr >= 0) {
			 khd->batching++;
		 ...
		 nr = team351635313269_kyber_get_domain_token(kqd, khd, hctx);
		 if (nr >= 0) {
			 team351635313269_kyber_flush_busy_kcqs(khd, khd->cur_domain, rqs);
 ...
 static struct request *team351635313269_kyber_dispatch_request(struct blk_mq_hw_ctx *hctx)
 {
	 struct kyber_queue_data *kqd = hctx->queue->elevator->elevator_data;
	 struct kyber_hctx_data *khd = hctx->sched_data;
	 struct request *rq;
	 ...
	 spin_lock(&khd->lock);
	 printk("In team351635313269_kyber_dispatch_request function\n");
	 /*
	  * First, if we are still entitled to batch, try to dispatch a request
	  * from the batch.
	  */
	 if (khd->batching < kyber_batch_size[khd->cur_domain]) {
		 rq = team351635313269_kyber_dispatch_cur_domain(kqd, khd, hctx);
		 if (rq)
			 goto out;
	 }
 ...
 static bool team351635313269_kyber_has_work(struct blk_mq_hw_ctx *hctx)
 {
	 struct kyber_hctx_data *khd = hctx->sched_data;
	 int i;
 ...
 static int team351635313269_kyber_async_depth_show(void *data, struct seq_file *m)
 {
	 struct request_queue *q = data;
	 struct kyber_queue_data *kqd = q->elevator->elevator_data;
 ...
 static int team351635313269_kyber_cur_domain_show(void *data, struct seq_file *m)
 {
	 struct blk_mq_hw_ctx *hctx = data;
	 struct kyber_hctx_data *khd = hctx->sched_data;
 ...
 static int team351635313269_kyber_batching_show(void *data, struct seq_file *m)
 {
	 struct blk_mq_hw_ctx *hctx = data;
	 struct kyber_hctx_data *khd = hctx->sched_data;
 ...
 static const struct blk_mq_debugfs_attr kyber_queue_debugfs_attrs[] = {
	 ...
	 {"async_depth", 0400, team351635313269_kyber_async_depth_show},
	 {},
 };
 ...
 static const struct blk_mq_debugfs_attr kyber_hctx_debugfs_attrs[] = {
	 ...
	 {"cur_domain", 0400, team351635313269_kyber_cur_domain_show},
	 {"batching", 0400, team351635313269_kyber_batching_show},
	 {},
 };
 #undef KYBER_HCTX_DOMAIN_ATTRS
 #endif
 
 static struct elevator_type team351635313269_kyber_sched = {
	 .ops = {
		 .init_sched = team351635313269_kyber_init_sched,
		 .exit_sched = team351635313269_kyber_exit_sched,
		 .init_hctx = team351635313269_kyber_init_hctx,
		 .exit_hctx = team351635313269_kyber_exit_hctx,
		 .limit_depth = team351635313269_kyber_limit_depth,
		 .bio_merge = team351635313269_kyber_bio_merge,
		 .prepare_request = team351635313269_kyber_prepare_request,
		 .insert_requests = team351635313269_kyber_insert_requests,
		 .finish_request = team351635313269_kyber_finish_request,
		 .requeue_request = team351635313269_kyber_finish_request,
		 .completed_request = team351635313269_kyber_completed_request,
		 .dispatch_request = team351635313269_kyber_dispatch_request,
		 .has_work = team351635313269_kyber_has_work,
		 .depth_updated = team351635313269_kyber_depth_updated,
	 },
 #ifdef CONFIG_BLK_DEBUG_FS
	 ...
	 .elevator_name = "Myteam",
	 .elevator_owner = THIS_MODULE,
 };
 
 static int __init team351635313269_kyber_init(void)
 {
	 return elv_register(&team351635313269_kyber_sched);
 }
 
 static void __exit team351635313269_kyber_exit(void)
 {
	 elv_unregister(&team351635313269_kyber_sched);
 }
 
 module_init(team351635313269_kyber_init);
 module_exit(team351635313269_kyber_exit);
 
 MODULE_AUTHOR("Team351635313269");
 MODULE_LICENSE("GPL");
 MODULE_DESCRIPTION("Kyber I/O customized scheduler");