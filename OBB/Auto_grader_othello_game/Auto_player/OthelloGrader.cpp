...
void OthelloGrader::gradeMoves(OthelloGame &game, int depth, int Pawn)
{
    int bestScore = -10000;
    int otherPawn, moveScore;
    Move bestMove;
    moveScores.clear();

    // Loop through all possible moves
    otherPawn = (Pawn == Pawn_O) ? Pawn_X : Pawn_O;
...
int evaluateBoardGrader(OthelloGame& game, int player) {
...
int OthelloGrader::minimax(OthelloGame &game, int depth, bool maximizingPlayer, int alpha, int beta, int Pawn, int pawnEval)
{
    ...
    if (depth == 0 || game.isGameOver())
    {
        ...
        return evaluateBoardGrader(game, pawnEval);
    }
