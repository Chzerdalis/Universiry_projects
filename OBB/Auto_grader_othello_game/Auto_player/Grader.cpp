...
int main()
{
    ...
    int finalScore;
    int depth;
    int aiPawn, humanPawn, currentPlayer;
    string firstPlayer;
    vector<string> moves;
   ...
   //Read from the json file names game_data.json and assign the values to the variables
    ...
    for_each(moves.begin(), moves.end(), [&](string move) {
        ...
        if(currentPlayer == 1){
            grader.gradeMoves(game, depth, currentPawn);
            ...
            //New way of calculating percentage
            double epsilon = 100;
            double k = 2.0;
            double student = playerMoveScore;
            double best = bestMoveScore;
            double bestDiv = 0;
            if (best == 0) {
                bestDiv = max(abs(best), epsilon);
            }else {
                bestDiv = abs(best);
            }
            //Exponential grading function
            percentage = exp(-k * abs(student - best) / bestDiv);

            percentages.push_back(percentage);

            std::cout << "Player " << currentPawn << " move: " << move << " score: " << playerMoveScore << " best score: " << bestMoveScore << " percentage: " << percentage << endl;
        }
    
        game.makeMove(row, col, currentPawn);
