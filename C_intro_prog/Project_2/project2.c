...
hash table and the int bucket size which i use in the sentinel
...
/*this is the struct of the hash table
table is the pointer to the array, size is the current size of the table
min_size is the starting size, n_elements the number of entries and 
largest_bucket the n_of_students of the biggest bucket*/
typedef struct {
  student_infoT **table;
  ...
  while(1) {
    ...
    switch (option) {
      ...
      case ('g') :{
        ...
        //checks the outputs and prints accordigly
        if(res == 1) {
          printf("\nG-OK %lu %hu\n", aem, course_id);
        }else if(res == -1) {
          printf("\nG-NOK %hu\n", course_id);
      ...
      case ('i') :{
        //searches for a students course
        scanf("%lu %hu", &aem, &course_id);
        if(aem == 0 || course_id == 0) {
          printf("\nInvalid input.\n");
          break;
      ...
      case ('t') :{
        //prints all entries from the hush table
        print_by_name(&hash_table);
        break;
      }
      case ('n') :{
        //searches for a students by their name in the hash table
        //and checks the size of the name inputted
        //prints comparisons
        scanf("%ms", &last_name);
        if(strlen(last_name) > NAMESIZE - 1) {
          last_name_help = (char*)realloc(last_name, NAMESIZE);
...
//clear has two modes, if the int is 1 it frees all the memory and then initializes the hash table
//if !=0 then it doesnt initialize
void clear(listT *list, hash_tableT *hash_table, const int init) {
  int i;
...
//it free all the memory of the hash table
void clear_hash_table(hash_tableT *hash_table) {
  int i;
...
//this funtion adds to the list that corresponds to the right bucket of the hash_table
int add_hash(hash_tableT *hash_table, student_infoT *student_to_add) {
  student_infoT *prev, *temp;
  double load_factor;
  //searches for the right place to add
...
int rmv_hash(hash_tableT *hash_table, student_infoT *student_to_rmv) {
  student_infoT *prev;
  double load_factor;
  //searches for the right place to remouve
...
//prints all the entries using the hash table
void print_by_name(hash_tableT *hash_table) {
  unsigned int i;
  student_infoT *curr;
  double load_factor;
...
/*this function is used to rehash the table it deletes the old table and creates 
a new one with the right size and from the list it adds one by one all the entries again*/
void rehash(hash_tableT *hash_table, listT *list, unsigned int size) {
  unsigned int i;
  int res;
