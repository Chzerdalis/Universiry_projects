...
int main(int argc, char **argv) {
    ...
    *h_OutputCPU;
    ...
    //To measure time ggor CPU we use the #include <sys/time.h> library
    ...
    //If not divisible we have to add one exta block
    dim3 grid_dim((imageW + 31) / 32, (imageH + 31) / 32);
    //MAX THREADS PER BLOCK 1024
    dim3 block_dim(32, 32);
	  printf("grid dims are %d %d", grid_dim.x, grid_dim.y);
