...
int main(int argc, char **argv) {
    ...
    extern int     optind;
           ...
           int     numClusters, numCoords, numObjs, numThreads, numChunks;
    ...
    while ( (opt=getopt(argc,argv,"p:i:c:n:t:x:abdohq"))!= EOF) {
        switch (opt) {
    ...
    printf("Running skmeans with %d threads and %d Chunks", numThreads, numChunks);
    seq_kmeans(objects, numCoords, numObjs, numClusters, threshold, membership,
               clusters, numThreads, numChunks);
