...
//	}
	...
	if(btn_oldvalue < btn_value) {
		if(btn_value % 2 == 1) {
			XGpio_WriteReg(0x43C00008, 0, 1);
			counter_value = 0;
			xil_printf("Number is: %d btn_value is: %d\r\n", counter_value, btn_value);
		}else if(btn_value != 0) {
			xil_printf("old_button is: %d btn_value is: %d\r\n", btn_oldvalue, btn_value);
			if((btn_value - btn_oldvalue) == 2) {
				XGpio_WriteReg(0x43C00000, 0, 1);
			}else if((btn_value - btn_oldvalue) == 4) {
				XGpio_WriteReg(0x43C00004, 0, bool_triger);
				bool_triger = (bool_triger) ? 0 : 1;
			}
			counter_value = counter_value + btn_value;
			xil_printf("Number is: %d btn_value is: %d\r\n", counter_value, btn_value);
		}else {
			xil_printf("Last case\r\n");
		}
	} else {
		xil_printf("Number is: %d btn_value is: %d\r\n", counter_value, btn_value);
		if((btn_value - btn_oldvalue) == -2) {
			XGpio_WriteReg(0x43C00000, 0, 0);
		}else if((btn_value - btn_oldvalue) == -1) {
			XGpio_WriteReg(0x43C00008, 0, 0);
		}
