...
# 3 "large_N/lsal.cpp" 2
# 1 "large_N/lsal.hpp" 1
# 19 "large_N/lsal.hpp"
__attribute__((sdx_kernel("compute_matrices", 0))) void compute_matrices(char *query, char *database, int *max_index, char *direction_matrix);
# 4 "large_N/lsal.cpp" 2
...
#pragma HLS BIND_STORAGE variable=diag_array_2 type=ram_t2p impl=bram
 ...
 short diag_array_3[(256) + 1];
#pragma HLS ARRAY_PARTITION variable=diag_array_3 dim=1 factor=32 cyclic
#pragma HLS BIND_STORAGE variable=diag_array_3 type=ram_t2p impl=bram

 short max_value_arr[(256)];
...
#pragma HLS BIND_STORAGE variable=direction_buff type=ram_t2p impl=bram
 ...
 MEM_1: memset(diag_array_1, 0, sizeof(short) * ((256)+1));
  MEM_2: memset(diag_array_2, 0, sizeof(short) * ((256)+1));
  MEM_3: memset(diag_array_3, 0, sizeof(short) * ((256)+1));

  MEM_4: memcpy(querry_buff, query, sizeof(char) * (256));
  MEM_5: memcpy(database_buff + 1, database, sizeof(char) * (256));
# 59 "large_N/lsal.cpp"
        MEM_8: memset(max_value_arr, 0, sizeof(int) * (256));
  MEM_9: memset(max_index_arr, 0, sizeof(int) * (256));

     OUTER_LOOP: for (k = 0; k < (65536 + 2 * ((256) - 1)) - (256) + 1; k = k + 1) {
#pragma HLS PIPELINE

 VITIS_LOOP_65_1: for(i = 1; i < (256); i++) {
       database_buff[i - 1] = database_buff[i];
      }
      database_buff[(256)-1] = database[k + (256) - 1];

         INNER_LOOP: for (i = 0; i < (256); i++) {


             j = (256) - 1 - i;
             short match = (querry_buff[j] == database_buff[i]) ? 2 : -1;
             short north = diag_array_2[i] + -1;
             short northwest = match + diag_array_1[i + 1];
             short west = diag_array_2[i + 1] + -1;
    ...
    diag_array_3[i] = max_value;
                ...
                if(max_value > max_value_arr[i]) {
                    ...
                    max_index_arr[i] = (i + k) * (256) + j;
                }
         }
         memcpy(direction_matrix + (k*(256)), direction_buff, sizeof(char)*(256));
         memcpy(diag_array_1, diag_array_2, sizeof(short)*((256)+1));
         memcpy(diag_array_2, diag_array_3, sizeof(short)*((256)+1));
     }
  MAX_LOOP: for(i = 0; i < (256); i++) {
#pragma HLS PIPELINE
