...
void compute_matrices_diag(char *query, char *database, int *max_index,
	...
	memset(diag_array_1, 0, sizeof(short) * (N+1));
	memset(diag_array_2, 0, sizeof(short) * (N+1));
	memset(diag_array_3, 0, sizeof(short) * (N+1));

	memcpy(querry_buff, query, sizeof(char) * N);
	memcpy(database_buff, database, sizeof(char) * M);

    for (int k = 0; k < M - N + 1; k = k + 1) {
        for (i = 0; i < N; i++) {
            j = N - 1 - i;
            ...
            max_value_ar[i] = max_value;
            max_index_ar[i] = (i+k) * N + j;

			// printf("max_value_ar[%d] = %d\n", i, max_index_ar[i]);
        }

		memcpy(diag_array_1, diag_array_2, sizeof(short) * (N+1));
		memcpy(diag_array_2, diag_array_3, sizeof(short) * (N+1));

		// printf("after memcpy");

        for(i = 0; i < N; i++) {
            j = N - 1 - i;
            if (max_value_ar[i] > global_max) {
                global_max = max_value_ar[i];
                max_index[0] = max_index_ar[i];
        	}
...
int main(int argc, char** argv) {
    ...
    t1 = clock();
	compute_matrices_diag(query, database, max_index, similarity_matrix, direction_matrix);
	t2 = clock();
	...
	for (int i = N - 1; i < M - N + 1; i++) {
		for (int j = 0; j < N; j++) {
			if(direction_matrix[i * N + j] != direction_matrix_golden[i * (N+1) + j + 1]) {
				printf("Error in direction matrix at (%d, %d)\n", i, j);
				return EXIT_FAILURE;
			}
		}
	}
	printf(" Direction matrix is correct\n");
