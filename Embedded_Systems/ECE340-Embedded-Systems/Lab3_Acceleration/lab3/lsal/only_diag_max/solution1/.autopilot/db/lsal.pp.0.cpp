...
# 3 "only_diag_max/lsal.cpp" 2
# 1 "only_diag_max/lsal.hpp" 1
# 19 "only_diag_max/lsal.hpp"
...
# 8 "only_diag_max/lsal.cpp"

  short max_value_temp = 0;
        int max_idx_temp = 0;

        int i, j, k;
        char diag_array_1[(32) + 1];
...
#pragma HLS BIND_STORAGE variable=direction_buff type=ram_t2p impl=bram
 ...
 MEM_1: memset(diag_array_1, 0, sizeof(char) * ((32)+1));
  MEM_2: memset(diag_array_2, 0, sizeof(char) * ((32)+1));
  MEM_3: memset(diag_array_3, 0, sizeof(char) * ((32)+1));
...
#pragma HLS PIPELINE
 database_buff[k] = database[k];
  ...
  MEM_9: memset(max_index_arr, 0, sizeof(int) * (32));
     ...
     OUTER_LOOP: for (k = 0; k < (65536 + 2 * ((32) - 1)) - (32) + 1; k = k + 1) {
#pragma HLS PIPELINE
 ...
 INNER_LOOP: for (i = 0; i < (32); i++) {
             j = (32) - 1 - i;
             signed char match = (querry_buff[j] == database_buff[i + k]) ? 2 : -1;
             signed char north = diag_array_2[i] + -1;
             signed char northwest = match + diag_array_1[i + 1];
             signed char west = diag_array_2[i + 1] + -1;
    ...
    diag_array_3[i] = max_value;
                ...
                if(max_value > max_value_temp) {
                 ...
                 max_idx_temp = (i + k) * (32) + j;
                }
         }
         memcpy(direction_matrix + (k*(32)), direction_buff, sizeof(char)*(32));
         memcpy(diag_array_1, diag_array_2, sizeof(char)*((32)+1));
         memcpy(diag_array_2, diag_array_3, sizeof(char)*((32)+1));




     }
# 121 "only_diag_max/lsal.cpp"
