...
int main( int argc, char* argv[] )
{
	...
	tot = (UINT*) calloc(noClusters, 4*sizeof(UINT));
    ...
    do {
	   ...
	   /* 2. Go through each pixel in the input image and calculate its nearest mean. 
			...
			for ( row = 0 ; row < height ; ++row)  {
				for ( col = 0 ; col < width ; ++col , ++k) {	

				
					// Get pixel's RGB values
					BMP_GetPixelRGB( bmp, col, row, &r, &g, &b );
					
					minDist = 200000; // Larger than the lagest possible value
					for (i = 0, y = -1; i < noClusters; ++i) {
     ...
     /* 3. Update the mean of each cluster based on the pixels assigned to them. */
      	if (noOfMoves > 0) {
			...
			means[i].r = (UCHAR) ((tot[4*i])/(tot[4*i + 3]));
			means[i].g = (UCHAR) ((tot[4*i + 1])/(tot[4*i + 3]));
			means[i].b = (UCHAR) ((tot[4*i + 2])/(tot[4*i + 3]));
		  	}
